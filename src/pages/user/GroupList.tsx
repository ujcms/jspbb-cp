...
const GroupList: FC = () => {
  ...
  const moveRow = async (dragIndex: number, hoverIndex: number) => {
    ...
    const updatedData = update(data, {$splice: [[dragIndex, 1], [hoverIndex, 0, dragRow]]});
  ...
  const columns = [
    ...
    {title: formatMessage({id: 'group.type'}), dataIndex: 'type', sorter: true, render: (val: any) => <FormattedMessage id={`group.type.${val}`}/>},
    {title: formatMessage({id: 'group.reputation'}), dataIndex: 'reputation'},
    {title: formatMessage({id: 'group.isTrusted'}), dataIndex: 'isTrusted', render: (val: any) => (val ? <CheckOutlined/> : <CloseOutlined/>)},
    {title: formatMessage({id: 'group.order'}), dataIndex: 'order', sorter: true, width: 100},
    {
      title: formatMessage({id: 'table.action'}),
      render: (text: any, record: any) => (
        <Space split={<Divider type="vertical"/>} size={0}><Access accessible={access['group:edit']}><a onClick={() => handleEdit(record)}><FormattedMessage id="edit"/></a></Access>
          {record.id > 10 && <Access accessible={access['group:delete']}><Popconfirm title={formatMessage({id: 'confirmDelete'})} onConfirm={() => handelDelete([record.id])}><a><FormattedMessage id="delete"/></a></Popconfirm></Access>}
        </Space>
      ),
    },
  ];
  return (
    <PageContainer header={{title: null}}>
      <Card bordered={false}>
        <div className={us.mb3}>
          <Form form={form} onFinish={handleSearch} layout="inline">
            <Form.Item name="Q_Contains_name" label={formatMessage({id: 'group.name'})}><Input/></Form.Item>
            <Form.Item><Space><Button type="primary" htmlType="submit"><FormattedMessage id="search"/></Button><Button onClick={handleReset}> <FormattedMessage id="reset"/></Button></Space></Form.Item>
          </Form>
        </div>
        <div className={us.mb3}>
          <Space>
            <Access accessible={access['group:add']}><Button type="primary" onClick={handleAdd}><PlusOutlined/> <FormattedMessage id="add"/></Button></Access>
        ...
        <DndProvider manager={React.useRef(createDndContext(HTML5Backend)).current.dragDropManager as any}>
          <Table
            rowKey="id"
            components={{body: {row: DraggableRow}}}
            onRow={(values: any, index?: number) => ({index, moveRow, onDoubleClick: () => handleEdit(values)})}
            loading={loading}
            rowSelection={{selectedRowKeys, onChange: (currentSelectedRowKeys) => setSelectedRowKeys(currentSelectedRowKeys), getCheckboxProps: (record) => ({disabled: record.id <= 10})}}
      ...
      <GroupForm formValues={formValues} valuesList={data ?? []} visible={modalVisible} setVisible={setModalVisible} refresh={run} moduleName="group"/>
    </PageContainer>
  );
};
